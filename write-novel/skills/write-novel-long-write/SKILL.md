...
**先查选题决策**：如果项目根存在 `选题决策.md`（write-novel-long-scan Phase 4 产出，开书前搬入），读取它——取排在最前（可行性最高）的推荐选题作为开书起点，向用户确认：「扫榜建议写 X（能爆的原因 Y，差异化 Z），按这个开书？」并看 `扫榜日期`：距今较久则提示"市场数据可能过期，建议复扫"。用户认可 → 带该选题的题材/卖点/差异化进入 Phase 2。
...
3. 如均不存在，提示用户：「对标书原文已放入 `对标/{书名}/原文/`。要先用 `/write-novel-long-analyze` 拆解吗？拆完黄金三章会先给你预览，确认后可继续全量拆解，拆完后 `拆文报告.md` 会自动存入 `拆文库/{书名}/`，写作时会自动按 `对标/ → 拆文库/` 顺序读取。」
...
4. **有对标书但 `文风.md` 缺失** → 日更文风召回 fail-fast，提示先运行 `/write-novel-long-analyze` Stage 6 并 `/write-novel-import` 同步；**完全无对标项目**则跳过文风召回，不阻塞
...
| 想对比参考书 | write-novel-long-analyze | `/write-novel-long-analyze` |
| 需要市场方向 | write-novel-long-scan | `/write-novel-long-scan` |
