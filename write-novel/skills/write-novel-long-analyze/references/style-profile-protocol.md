...
- **读**：write-novel-import 同步到 `{项目}/对标/{书名}/文风.md`；write-novel-long-write 读项目对标视图（回退拆文库）
...
- `文风可用：否：{原因}` → 文风质量不足（例如原文缺失、锚点全是占位符）。write-novel-story-researcher 读取时返回 `gaps.profile_degenerate: true`，narrative-writer 跳过文风，按默认 Gates 写作，避免被误导。
- 兼容旧文件：若旧 `文风.md` 出现旧版内部降级标记，write-novel-story-researcher 也按“文风可用：否”处理。
...
精确决议表见 `.claude/agents/write-novel-narrative-writer.md` 的“被调用协议”段。
