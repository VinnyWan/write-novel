...
深度分析阶段调用现成的拆解管道，不重新发明：长篇运行 `/write-novel-long-analyze` 的完整拆解管道，短篇运行 `/write-novel-short-analyze` 的拆解管道。拆解方法论与输出模板由对应 analyze skill 自带，write-novel-import 不执行拆解方法论、不维护这些文件。
...
2. **用户确认**：
   ...
   - **最后一章是否完整**：完整章 / 残稿（写了一半）。若是残稿，提示用户并把「残稿到第 N 章」记入上下文，让用户决定是「基于残章续写」还是「先补完再导入」。write-novel-import 只记录用户决定，不替用户选。
3. **输出确认**：向用户展示检测到的章节范围、字数、判定的篇幅类型、最后一章状态，确认后开始分析

### 环境检测前置

在进入 Phase 2 之前，先检测项目是否已部署 write-novel-setup 基础设施：
...
> 「检测到当前项目尚未部署写作基础设施。建议先运行 `/write-novel-setup` 再回来导入，否则深度分析阶段无法使用并行 deconstruction-agent agent。」
...
原文备份由 Phase 2 调用的 analyze 拆解管道负责（analyze 管道前置步骤会把原文复制/保存到 `拆文库/{书名}/原文/`，对应 write-novel-long-analyze 与 write-novel-short-analyze 的「原文备份（管道前置步骤）」）。Phase 1 只需确认源文件就绪（路径有效或文本已拿到），不在此处单独备份，避免与 analyze 管道重复备份逻辑。
...
| 篇幅 | 调用的拆解管道 | 产物目录 |
|------|--------------|---------|
| 长篇 | write-novel-long-analyze 的 6 阶段管道（Stage 0-5） | `拆文库/{书名}/` |
...
write-novel-long-analyze 在 Stage 0+1（黄金三章）后会**自动停靠**并用 AskUserQuestion 询问是否继续全量拆解（对应 write-novel-long-analyze 的「Stage 1 停靠点」）。但导入场景需要 Stage 2-6 的全套产物（逐章摘要 / 聚合分析 / 设定关系 / 汇总报告 / 文风），缺一不可——否则 Phase 3 迁移会拿到半成品。
...
长篇分析输出到 `拆文库/{书名}/`，与 write-novel-long-analyze 拆解管道完全一致：
...
短篇分析输出到 `拆文库/{书名}/`，与 write-novel-short-analyze 拆解管道一致：
...
| 6 | 文风 | 拆文报告.md + 章节/第1-3章_深度拆解.md + 章节/*_摘要.md + 原文/原文.txt | 文风.md（整书级写作技法视图，write-novel-long-write 日更循环必读） | 文风落盘 `拆文库/{书名}/文风.md` |

### 短篇拆文管道

> 管道详细说明见 write-novel-short-analyze（运行 `/write-novel-short-analyze`），此处仅列概要。
...
沿用 write-novel-long-analyze 的分块策略（Stage 2 使用 deconstruction-agent agent 并行，其他阶段按以下策略分块）：
...
- 新会话读取进度文件定位断点
- 从断点所在块的起始章节恢复
- 长篇进度文件格式沿用 write-novel-long-analyze 拆解管道的进度段落约定，包含当前阶段、最后处理章节、已完成阶段列表、更新时间
...
将 `拆文库/{书名}/` 的短篇拆文产物迁移为 `{短篇标题}/` 短篇工程结构，供 write-novel-short-write Phase 3 逐场景写作无缝接手。迁移规则详见 [references/structure-mapping-short.md](references/structure-mapping-short.md)。
...
本 skill 自带的 reference 文件全部位于 `references/`，按场景加载。涉及别的 skill 的方法论/模板时，write-novel-import 不直接加载文件，而是运行对应 `/命令` 由该 skill 自行加载。
...
| 章节格式识别 | 由 write-novel-long-analyze 拆解管道（运行 `/write-novel-long-analyze`）的阶段 1 负责 |

### Phase 2：深度分析

| 场景 | 加载文件 / 相关 skill |
|------|---------|
| 长篇深度分析（方法论、质量门控、输出模板均自带） | 运行 `/write-novel-long-analyze` 调用长篇拆解管道 |
| 短篇深度分析（方法论、质量门控、输出模板均自带） | 运行 `/write-novel-short-analyze` 调用短篇拆解管道 |
...
| 场景 | 说明 |
|------|---------|
| 长篇项目结构规范 | 参见 write-novel-long-write（Phase 4 项目文件结构） |
| 短篇项目结构规范 | 参见 write-novel-short-write（Phase 3 项目结构） |
| 环境部署 | 部署模板由 `/write-novel-setup` 提供，write-novel-import 不负责部署 |
...
2. 问题：自动处理项（别名合并、低置信度标记）、建议检查项、必须处理项
