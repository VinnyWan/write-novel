...
**文件名约定**：`拆文报告.md / 情节节点.md / 写作手法.md` 由 `write-novel-short-write` 硬编码
...
### 7.3 `write-novel-short-analyze/references/output-templates.md` BLOCK 项扫描

扫 `write-novel-short-analyze/references/output-templates.md` 所有 `[BLOCK]` 标注项对应的产出段是否在 `拆文报告.md` 出现。
任一缺失 → 阻断。`[WARN]` 项 → 写入拆文报告末尾「待补」清单，不阻断。

### 7.4 通过

清空 `_meta.json.last_stage_in_progress`，append `6` 到 `stages_completed[]`，提示
用户「拆解完成，可调用 `/write-novel-short-write` 写下一篇」。

---

## 下游消费规范（write-novel-short-write 怎么用）

> `write-novel-short-write` 当前硬编码读 `拆文报告.md / 情节节点.md / 写作手法.md` 三个 markdown。
...
/write-novel-short-write 拆文库/{书名}/
