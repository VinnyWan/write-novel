...
- 通用称呼黑名单：同长篇 deconstruction-agent 的黑名单（老师、老板、师傅等）
...
- **output-contract.md** = 输出契约 SSOT：定义 Stage → 文件映射 + `_meta.json` schema（含 `structure_counts`）+ 下游 `write-novel-short-write` 消费规范 + Phase 7 门控接入点。与 `write-novel-short-write` 副本字节一致，由 `scripts/check-shared-files.sh` 守护
