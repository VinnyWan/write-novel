---
name: write-novel-short-analyze
version: 3.0.0
description: |
  短篇网文拆文。拆解爆款短篇小说（番茄短篇 / 故事会 / 知乎盐选 / 追妻 / 世情 / 重生 /
  虐渣等通俗题材）的故事核、结构、情感线、反转设计、写作手法、共鸣层次。
  单一全量拆解管道：跑完 Stage 2-6 产出完整拆文报告，落盘到 `拆文库/{书名}/`，
  下游 `write-novel-short-write` 同时读拆文报告 + 情节节点 + 写作手法 + 原文 + _meta.json 写下一篇。
  触发方式：/write-novel-short-analyze、/短篇拆文、「拆短篇」「拆这篇短文」「短篇拆文」
  「精细拆解短篇」「8000 字短篇拆解」「番茄短篇拆文」「故事会拆解」「盐言故事拆解」
  「分析这篇短篇」——均进入同一管道。（旧触发词：/story-short-analyze）
metadata:
  openclaw:
    source: https://github.com/worldwonderer/oh-story-claudecode
---

# write-novel-short-analyze：短篇网文拆文
...
word_count = 全文字数
  ...
  └─ > 20,000          → 提示「此文字数 {N} 偏长，建议改用 /write-novel-long-analyze。
                           仍要按短篇拆请明确回复『按短篇继续』」
```
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 准备开写 | write-novel-short-write（同时读 拆文报告.md + 情节节点.md + 写作手法.md + 原文/ + _meta.json） | `/write-novel-short-write` |
| 需要市场数据 | write-novel-short-scan | `/write-novel-short-scan` |
| 字数 > 20k 更适合长篇 | write-novel-long-scan → write-novel-long-analyze | `/write-novel-long-scan` |
