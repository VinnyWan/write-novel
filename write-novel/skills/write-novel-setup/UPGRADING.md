...
推荐：运行 `/write-novel-setup` 重新部署，自动走合并策略。

## 文件分类

### 可安全覆盖

这些文件由 write-novel-setup 管理，不含用户自定义内容：
- `.claude/hooks/` — 所有 hook 脚本与 `lib/` 辅助库
- `.claude/agents/` — 所有 agent 定义
- `.claude/rules/` — 所有 path-scoped 规则
- `.claude/skills/write-novel-setup/references/agent-references/` — Agent 参考资料副本
...
- `agents_version: 2` → 旧版，需重新部署以获取 write-novel-story-explorer agent
- `agents_version: 3` → 旧版，需重新部署以获取 write-novel-story-explorer agent
...
- 4 个创作型 Agent + 1 个研究型 Agent（write-novel-story-architect, character-designer, narrative-writer, reviewer, write-novel-story-researcher）
...
- 新增 write-novel-story-researcher 只读查询 Agent（角色/伏笔/设定/进度查询，日更上下文快速加载）
- 6 个 Agent 总计（write-novel-story-architect, character-designer, narrative-writer, reviewer, write-novel-story-researcher）
- write-novel-story-researcher 被 write-novel-long-write、write-novel-review、story 路由集成调用

### v4

- 新增 deconstruction-agent 章节提取 Agent
- 7 个 Agent 总计（write-novel-story-architect, character-designer, narrative-writer, reviewer, write-novel-story-researcher, deconstruction-agent）
...
- 已部署项目重新运行 `/write-novel-setup` 后获取新版 agent 定义
...
- 修复长篇 `/write-novel-long-write 日更` 批量续写中的 continuation 规则：同一批次内“继续/续写/日更”保持在 daily workflow，不直接跳到正文续写。
- 修复 `detect-story-gaps.sh` 对伏笔表头和正常开放伏笔（`未埋`/`已埋`）的误报；SessionStart 只提示 `已过期` 或异常状态。
- 已部署项目需重新运行 `/write-novel-setup`，以覆盖 `.claude/hooks/`、`.claude/agents/`、`.claude/rules/` 并获得新版 hook 行为。

### v8

- 修复 write-novel-review 及部署后的 reviewer Agent 在项目根目录下读取参考文件时，只找裸文件名（如 `quality-checklist.md`）导致找不到 skill references 的问题。
- Agent 模板新增参考文件路径规则：优先从 `.claude/skills/` 或 `skills/` 拼接解析 `write-novel-setup/references/agent-references/*.md` 规范路径，避免依赖当前工作目录且不跨 skill 引用 references。
- 已部署项目需重新运行 `/write-novel-setup`，以覆盖 `.claude/agents/` 并获得新版参考文件路径规则。
...
- Agent Reference bundle 补齐并 canonicalize：
  - `genre-readers.md`：从 `write-novel-long-write/references/genre-readers.md` 复制为 write-novel-setup canonical 副本。
  - `genre-writing-formulas.md`：从 `write-novel-long-write/references/genre-writing-formulas.md` 复制为 write-novel-setup canonical 副本。
  - `emotional-methods.md`：从 `write-novel-long-write/references/emotional-methods.md` 复制为 write-novel-setup canonical 副本。
  - `style-combat-face.md`：从 `write-novel-long-write/references/style-combat-face.md` 复制为 write-novel-setup canonical 副本。
...
已部署项目请重新运行 `/write-novel-setup`，刷新写作 Agent；主要影响是日更续写更稳定地沿用对标文风。

### v11

**v0.3.0: Agent 架构统一。** 31 个 skill 合并为 15 个规范 skill，15 个 agent 合并为 6 个规范 agent。
- Agent 合并：context-agent → write-novel-story-architect；chapter-extractor → deconstruction-agent；consistency-checker → reviewer；write-novel-story-explorer → write-novel-story-researcher；write-novel-story-researcher + data-agent 统一。
- 所有旧 skill 命名空间（write-novel-* / webnovel-*）废弃，统一为 story-*。
- 旧触发词作为别名保留在路由表中，向后兼容。
- 已部署项目需重新运行 `/write-novel-setup` 以覆盖 `.claude/agents/`。
