...
if [ ! -f "$HOOK_DIR/lib/common.sh" ] || [ ! -f "$HOOK_DIR/lib/sentinel.sh" ]; then
  printf '%b' "[WARN] story hook 函数库缺失。重新运行 /write-novel-setup 恢复 .claude/hooks/lib/。\n"
  exit 0
fi
...
if sentinel_exists "$ROOT/.story-deployed"; then
  ...
  if [ -n "$MISSING_HOOKS" ]; then
    ...
    OUTPUT+="  修复：重新运行 /write-novel-setup 恢复缺失的 hook。\n\n"
    HAS_CONTENT=true
  fi
  ...
  case "$AGENTS_VERSION" in
    ''|*[!0-9]*)
      OUTPUT+="[WARN] .story-deployed 缺少数字 agents_version。重新运行 /write-novel-setup。\n\n"
      HAS_CONTENT=true
      ;;
    *)
      if [ "$AGENTS_VERSION" -lt 10 ]; then
        OUTPUT+="[WARN] write-novel-setup agents_version=$AGENTS_VERSION 低于 v10。重新运行 /write-novel-setup 刷新 hooks、agents 和 references。\n\n"
        HAS_CONTENT=true
      fi
      ;;
  esac
  ...
  for field in setup_skill_version target_cli resolver_strategy references_dir; do
    if [ -z "$(read_sentinel_field "$field" "$ROOT/.story-deployed")" ]; then
      OUTPUT+="[WARN] .story-deployed 缺少 $field 字段。重新运行 /write-novel-setup 刷新部署元信息。\n\n"
      HAS_CONTENT=true
    fi
  done
  ...
  if [ -n "$REFERENCES_DIR" ]; then
    ...
    if [ ! -d "$REFERENCES_PATH" ] || ! find "$REFERENCES_PATH" -maxdepth 1 -type f -name "*.md" -print -quit 2>/dev/null | grep -q .; then
      OUTPUT+="[WARN] write-novel-setup 参考资料包缺失或为空：${REFERENCES_DIR}。重新运行 /write-novel-setup。\n\n"
      HAS_CONTENT=true
    fi
  fi
else
...
if [ -d "$ROOT/拆文库" ]; then
  ...
  if [ "$PROGRESS_COUNT" -gt 0 ]; then
    OUTPUT+="[INFO] 拆文库/ 中有 $PROGRESS_COUNT 个未完成拆文。运行 /write-novel-long-analyze 或 /write-novel-short-analyze。\n"
    HAS_CONTENT=true
  fi
fi
