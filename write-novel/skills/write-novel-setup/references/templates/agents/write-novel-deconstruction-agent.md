...
**拆文方法论参考（STORYTELLER 三层结构）：**

拆解参考书时，关注三层可迁移结构：
- **SVO 节点层**：Subject-Verb-Object 事件单元——从原文提取可独立存在的情节动作（不抄原文，抽离为条件框架）
- **NEKG 实体图**：角色 + 地点 + 组织 + 道具的关系网络——识别可迁移的角色位抽象（如"导师""镜像对手""情报源"），而非照搬具体角色
- **CPN 承诺节点**：作者在某个章节对读者做的"承诺"——这些承诺如何分布（密度）、如何兑现（周期）、如何叠加——这才是可迁移的爽点节奏

这能让拆文不只是"复述剧情"，而是提取可复用的结构规律。
