...
## {角色名}
- **当前身份**：{最新身份/职业}
- **当前能力**：{最新能力水平}（功法技能树详见 [功法状态](功法状态.md#{角色名})）
- **关键关系**：
...
1. 每章写完后，检查本章是否引起了任何角色的状态变化（身份、能力、关系、公众形象）。若涉及功法技能获得或升级，同步更新 `追踪/功法状态.md`
...
Phase 4 Stage F1 执行追踪原子更新，所有追踪文件在一次操作中按依赖顺序更新：

1. **伏笔**（`追踪/伏笔.md`）：新增/回收伏笔 — 最优先，无依赖
2. **时间线**（`追踪/时间线.md`）：记录事件时序 — 可能引用伏笔条目
3. **角色状态**（`追踪/角色状态.md`）：身份/能力/关系/公众形象变化 — 可能引用时间线事件
4. **功法状态**（`追踪/功法状态.md`）：功法获得或阶段升级 — 可能引用角色状态中的能力字段

原子更新原则：
- 以上 4 个文件在一次操作中全部更新，减少中间状态
- 后续文件可引用前序文件的更新结果（如功法状态引用角色状态中的能力变更）
- 任一文件更新失败时，已成功的文件保留，失败的标记并在 commit 中记录
