...
- `文风可用：否：{原因}` → 文风质量不足（例如原文缺失、锚点全是占位符）。story-researcher 读取时返回 `gaps.profile_degenerate: true`，narrative-writer 跳过文风，按默认 Gates 写作，避免被误导。
- 兼容旧文件：若旧 `文风.md` 出现旧版内部降级标记，story-researcher 也按“文风可用：否”处理。
