...
**检测方式**：Gate A/B/D 由 agent 读取禁词表和句式模板进行匹配检测，Gate C/E/F 由 agent 语义判断。不依赖外部脚本。
...
加载 [references/banned-words.md](references/banned-words.md) 作为基础词表，同时加载 `references/methodology/banned-words-star-rating.md` 获取 1-5 星危险等级分类。检测时按星级优先级处理：5星（最毒句式）和 4星（动作修饰词）为最高优先级，出现必须替换；3星（表达/心理类）每章不超过 1 次；2星（判断/描述类）和 1星（连结/过渡类）标记并建议简化。
