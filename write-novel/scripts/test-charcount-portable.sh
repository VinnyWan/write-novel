...
# Microsoft Store 占位程序场景下都能正确数出中文字符数。（移植自 oh-story）
#
# 背景：Windows 上 python.org 安装后 `python3` 会落到 Microsoft Store 占位程序、
# 以 exit 49 静默失败，必须按 python3 -> python -> py 探测出真正可用的解释器。
# GitHub windows-latest 自带可用的 python3，因此 `--stub` 模式人为塞入一个
# exit-49 的假 python3 来复现真实故障。
#
# 用法：
...
# === 探测 + 统计命令（与技能文档逐字一致）===
# 用相对路径统计（先 cd 进书目录，再传文件名）：Windows Git Bash 下若把绝对 POSIX
# 路径直接喂给原生 Windows python，会被解析成 C:\tmp\... 而找不到文件；相对路径按
# 子进程真实 cwd 解析，三平台一致。
