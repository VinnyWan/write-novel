...
def cmd_init(args):
    ...
    # 复制追踪模板
    tracking_templates = [
        '追踪_上下文.md', '追踪_时间线.md', '追踪_角色状态.md',
    ]
    project_tmpl_dir = os.path.join(templates_dir, 'project')
    for tmpl in tracking_templates:
        src = os.path.join(project_tmpl_dir, tmpl)
        if os.path.isfile(src):
            dst = os.path.join(project_root, '追踪', tmpl.replace('追踪_', ''))
            if not os.path.exists(dst):
                shutil.copy2(src, dst)

    # 复制对标模板
    src = os.path.join(project_tmpl_dir, '对标目录.md')
    if os.path.isfile(src):
        dst = os.path.join(project_root, '对标', 'README.md')
        if not os.path.exists(dst):
            shutil.copy2(src, dst)
    ...
    print(f'  2. {os.path.join(project_root, "设定/世界观/")} — 创建世界观.md')
    print(f'  3. {os.path.join(project_root, "设定/角色/")} — 创建角色卡片')
    print(f'  4. {os.path.join(project_root, "大纲/")} — 规划大纲')
    print(f'  5. {os.path.join(project_root, "追踪/")} — 追踪上下文/时间线/角色状态')
...
def cmd_search(args):
    ...
    if args.chapter and args.volume:
        outline_path = ensure_nfc(os.path.join(
            project_root, '大纲', f'第{args.volume}卷_细纲_第{args.chapter}章.md'
        ))
...
def cmd_status(args):
    ...
    draft_dir = ensure_nfc(os.path.join(project_root, '正文'))
    draft_count = 0
