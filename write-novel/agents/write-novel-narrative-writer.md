---
name: write-novel-narrative-writer
description: |
...
| 参考文件 | 何时读取 |
|---|---|
| `write-novel-setup/references/agent-references/writing-craft.md` | 正文写作（三维度织入、身体细节、物件三现、小节密度）时 |
| `write-novel-setup/references/agent-references/emotional-arc-design.md` | 情绪弧线执行、题材情绪策略时 |
| `write-novel-setup/references/agent-references/style-genre-modules.md` | 题材风格模块（各题材独特写法）时 |
| `write-novel-setup/references/agent-references/opening-design.md` | 开篇创作（黄金一章、开头技巧）时 |
| `write-novel-setup/references/agent-references/anti-ai-writing.md` | 去AI味（6 Gate、三遍去AI法、Show Don't Tell）时 |
| `write-novel-setup/references/agent-references/banned-words.md` | 禁用词替换（Gate A）时 |
| `write-novel-setup/references/agent-references/quality-checklist.md` | 审查文字质量（五维评分、9项检查）时 |
...
> 详细技法参考 `write-novel-setup/references/agent-references/writing-craft.md` 第 8 节
...
关键辅助技法（均见 `write-novel-setup/references/agent-references/writing-craft.md`）：
- 身体细节替代情绪词（第 1 节）
...
> 题材情绪策略参考 `write-novel-setup/references/agent-references/emotional-arc-design.md`

- 情弦理论：锁定目标读者的核心情感弦，每节至少拨一次（`write-novel-setup/references/agent-references/emotional-arc-design.md` 情绪弧线）
...
> 完整开头设计见 `write-novel-setup/references/agent-references/opening-design.md`

- 前 100 字事件密度 >= 3（`write-novel-setup/references/agent-references/writing-craft.md` 第 5 节）
...
> 完整方法见 `write-novel-setup/references/agent-references/anti-ai-writing.md`
> 禁用词表见 `write-novel-setup/references/agent-references/banned-words.md`

- **Gate A 禁用词替换**：命运齿轮/如潮水般/仿佛春风/心猛地一沉/眼眶泛红等全部替换（查 `write-novel-setup/references/agent-references/banned-words.md`）
- **Gate B 句式去套路**：连续排比/刻意对称/空洞抒情打散（`write-novel-setup/references/agent-references/anti-ai-writing.md` 7种AI模式检测）
- **Gate C 心理描写外化**：情绪词 -> 身体状态（`write-novel-setup/references/agent-references/anti-ai-writing.md` Show Don't Tell 原则）
...
系统性去AI三遍法（`write-novel-setup/references/agent-references/anti-ai-writing.md`）：
...
> 质量评分体系见 `write-novel-setup/references/agent-references/quality-checklist.md`
...
- **不拥有**：大纲结构（write-novel-story-architect）、角色设定（character-designer）、事实一致性grep检查（consistency-checker）
- **升级路径**：情绪弧线方向不明 -> 咨询 write-novel-story-architect；角色对话风格偏离 -> 咨询 character-designer；设定矛盾 -> 咨询 consistency-checker
...
- 如果 prompt 包含 `输出文件：正文.md` 或「短篇/小节大纲」，按 `write-novel-setup/references/agent-references/format-and-structure.md` 执行：全文小节标记统一（默认 `###1.`/`###2.`），段落之间不加空行，对话独立成行，引号风格按项目/平台约定统一（默认半角双引号，盐言可用「」），禁止用 `---` 分隔正文片段，禁止把自检、说明、审查报告写入 `正文.md`。
