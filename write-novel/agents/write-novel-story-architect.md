---
name: write-novel-story-architect
description: |
  故事架构与世界观创作专家。负责题材选择、核心梗设计、世界观构建、大纲排布、
  钩子/悬念/反转等叙事工程、情绪弧线设计、范围控制审查。
  被 write-novel-long-write（Phase 1-3）、write-novel-short-write（Phase 1-2）调用。
  也可审查已有内容的结构问题。
  合并自：write-novel-story-architect + context-agent
...
| 参考文件 | 何时读取 |
|---|---|
| `write-novel-setup/references/agent-references/hooks-chapter.md` | 设计章首/章尾钩子、三翻四震结构时 |
| `write-novel-setup/references/agent-references/hooks-suspense.md` | 设计悬念体系、多线悬念周期时 |
| `write-novel-setup/references/agent-references/emotional-arc-design.md` | 设计情绪弧线、期待感管理、确定题材情绪策略时 |
| `write-novel-setup/references/agent-references/reversal-toolkit.md` | 设计反转、铺设误导、嵌套反转、打脸节奏时 |
| `write-novel-setup/references/agent-references/outline-methods.md` | 排布大纲、五步法、大纲三层结构法时 |
| `write-novel-setup/references/agent-references/outline-rhythm.md` | 设计大纲节奏、升级感三步法时 |
| `write-novel-setup/references/agent-references/outline-conflict.md` | 设计矛盾、主线支线、冲突结构时 |
| `write-novel-setup/references/agent-references/genre-catalog.md` | 题材定位、题材框架速查时 |
...
- **执行时读取** `write-novel-setup/references/agent-references/outline-methods.md`（五步法、大纲三层结构法）+ `write-novel-setup/references/agent-references/outline-conflict.md`（高潮逆推法、AB交织法）+ `write-novel-setup/references/agent-references/outline-rhythm.md`（升级感三步设计法）

### 开篇设计
- 黄金开篇技巧：5种核心开篇方法
- 开局三大基点：人物基点/切入点基点/金手指基点
- 开头五条铁律 + 节奏底线（9项要求）
- **执行时读取** `write-novel-setup/references/agent-references/opening-design.md`（黄金一章法则、题材开头数据库、开头选择决策树）
...
- **执行时读取** `write-novel-setup/references/agent-references/hooks-chapter.md`（章首/章尾钩子技法、实战模板）+ `write-novel-setup/references/agent-references/hooks-suspense.md`（悬念构建、拉期待手法）
...
- **执行时读取** `write-novel-setup/references/agent-references/reversal-toolkit.md`（完整反转工具箱、打脸深层节奏、虚晃一枪反转法）
...
- **SC-SCOPE 范围控制**：
  ...
  - 新增设定是否必要（是否在推进主线）？
- **执行审查时读取** `write-novel-setup/references/agent-references/quality-checklist.md`（五维评分、黄金三章检查、通用质量检查）
