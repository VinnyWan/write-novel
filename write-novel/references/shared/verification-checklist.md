...
- [ ] 2.2 运行 `/story-long-write {N} --resume` —— 应跳过已完成步骤，仅执行未完步骤
...
- [ ] 4.1 story-long-write 写完一章后的输出是否符合 3 段式报告（完成状态/问题/下一步）？
- [ ] 4.2 story-review 输出是否包含 S1-S4 严重度汇总和下一步命令？
- [ ] 4.3 story-deslop 输出是否包含 AI 味定级和修改统计？
...
- [ ] 5.4 写第 5 章时，story-architect 的 brief 是否包含「历史模式参考」小节？（如 project_memory.json 有匹配模式）
- [ ] 5.5 运行 `/story-doctor` —— 应包含 project_memory.json 健康检查（文件完整性、各类条目数）
