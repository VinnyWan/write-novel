...
> **定位**：网文写作/拆书通用质量自检清单。写作 skill 用于自检成稿质量；拆书 skill 用于评估源文成文质量。
> 拆书管道本身的质量门控见各拆书 skill 的 `output-templates.md`「质量门控必填字段」；所有数值阈值的唯一权威定义见各 skill 的 `material-decomposition.md`「质量标准」与「节奏分析」，本清单不自定义数值。
>
> **三类质量的分工（别混用）**：
>
> 1. **拆书管道质量门控**（执行过程中）→ 各拆书 skill 的 `output-templates.md`「质量门控必填字段」。每条带 `[BLOCK]` / `[WARN]` 标注；`[BLOCK]` 缺失 → Phase 7.3 阻断。
...
> **拆文管道完成度** 走各拆书 skill 的 `output-templates.md`「质量门控必填字段」的 `[BLOCK]` / `[WARN]` 清单。
